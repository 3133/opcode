...
namespace OPCClient
{
    class Program
    {
        ...
        public void work()
        {
            ...
            if (ConnectRemoteServer("192.168.2.98", "Matrikon.OPC.Simulation.1"))//本机
            {
                if (CreateGroup())
                {
                    Thread.Sleep(1000);//暂停线程以让DataChange反映，否则下面的同步读可能读不到
                                       //以下同步写
                   ...
                   // var q1 = ItemValues;//转换后获取item值
                    ...
                    //double q3 = Convert.ToDouble(ItemValues);//转换后获取item值，为防止读到的值为空，不用ItemValues.ToString()

                    Console.WriteLine("~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
                    Console.WriteLine("0-{0}", q0);
        ...
        {
            //获取本地计算机IP,计算机名称
            //strHostName = Dns.GetHostEntry("192.168.2.98").HostName;
            //或者通过局域网内计算机名称
            strHostName = "DESKTOP-S13B96M";
        ...
        private bool CreateGroup()
        {
            try
            {
                ...
                foreach (OPCGroup item in MyGroups)
                {
                    Console.WriteLine("++++++++++++++++" + item.Name);
                }
                MyGroup = MyServer.OPCGroups.Add("组1");//添加组
                                                       ...
                                                       //以下设置组属性
                {
                    ...
                    MyServer.OPCGroups.DefaultGroupUpdateRate = 3000;//默认组群的刷新频率为200ms
                    MyGroup.UpdateRate = 3000;//刷新频率为1秒。
        ...
        private void AddGroupItems()//添加组
        {
            ...
            //添加item如果创建组，用组名加项名字
            MyItem[0] = MyItems.AddItem("组1.TAG1", 0);//byte
            //MyItem[1] = MyItems.AddItem(".12", 0);//byte
            //MyItem[1] = MyItems.AddItem("Random.Int2", 1);//short
            //MyItem[2] = MyItems.AddItem("Random.Int3", 2);//bool
            //MyItem[3] = MyItems.AddItem("Random.Real4", 3);//string
            //移除组内item
            //Array Errors;
